...
The native installer review branch provides `/getting-started/`, an inspectable `/install.kujo`, and a checksum-pinned `/install.sh`. These are generated by the pinned Kujo-native Kennel publisher; do not hand-edit them. The site retains a preview notice until the latest stable Kennel archive contains installer protocol 2. Historical package artifacts and exact version metadata are unchanged.

Public bootstrap requires a compatible Kujo runtime with `--isolated-imports`, `file_lock`, `file_unlock`, `exec_process`, `symlink_atomic`, and `path_owned`, followed by the reviewed Kennel 1.1.0 release. The currently published Kujo 1.3.1 lacks the required capabilities. Python is no longer a consumer or publishing prerequisite. The pinned workflow builds/caches its exact Kujo runtime; Git and the GitHub CLI remain publishing tools. No release is created by this branch. Review the companion Kennel `docs/registry/release-candidate.md` for local installation and the release order.
