...
The central implementation lives in `kujolang/kennel/scripts/registry` and `.github/workflows/publish-kennel-package.yml`. Only published GitHub Releases qualify. Tag pushes do not publish packages. The registry requests reconciliation every 15 minutes (GitHub may delay scheduled runs) and supports manual `releases.yml` dispatch with optional package and existing release ID. Both use the same builder; no test releases are created.
...
Production activated 2026-09-08. The Git-connected Pages project is `kennel-registry`; the verified deployment settings are recorded in `cloudflare-pages.json`. DNS and TLS are active for `kennel.kujolang.ai`. Kennel 1.1.0 requires the published Kujo 1.4.0 runtime or newer; historical Kennel package releases retain their original client behavior. No historical release was rewritten to include new registry support.
...
The native publisher provides `/getting-started/`, an inspectable `/install.kujo`, and a checksum-pinned `/install.sh`. These are generated by the pinned Kujo-native Kennel publisher; do not hand-edit them. The site retains a preview notice until the latest stable Kennel archive contains installer protocol 2. Historical package artifacts and exact version metadata are unchanged.

Public bootstrap requires Kujo 1.4.0 or newer, followed by Kennel 1.1.0 or newer. Kujo 1.4.0 supplies the isolated imports, locking and process/filesystem operations required by the native installer. Python is no longer a consumer or publishing prerequisite. The pinned workflow builds/caches its exact Kujo runtime; Git and the GitHub CLI remain publishing tools. Reconciliation consumes actual published releases; it never creates them. Review the companion Kennel `docs/registry/release-candidate.md` for release verification.
